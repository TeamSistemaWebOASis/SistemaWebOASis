        ...
        $.ajax({
            ...
            if (data.strEstadistica.length != 0) {
                ...
                function drawChart() {
                    ...
                    var tituloGrafica = $('#txtPeriodoA').text().trim();

                    var options = {
                        title: (tituloGrafica == "") ? 'EVALUACIONES DEL PERIODO ACADÉMICO:' + " " + $('#ddlLstPeriodosEstudiante option:selected').text().trim()
                                                    : 'EVALUACIONES DEL' + tituloGrafica.toUpperCase(),
                        width: 920,
                        height: 350,
